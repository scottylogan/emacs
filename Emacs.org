...
When running in a GUI, set the default  frame size ( 45 rows x 140 columns), set the title to the filename, and set the default font; the size is set according to screen resolution. Finally, bind =scotty/new-empty-frame= to a global key (⌘N on MacOS)
...
#+begin_src emacs-lisp :tangle ./gui.el :noweb yes
  (when (or (eq window-system 'ns) (eq window-system 'x))
    ...
    (add-to-list 'initial-frame-alist ''(ns-tool-bar-visible . nil))
...
#+begin_src emacs-lisp
  ...
  ;; needed for batch mode - emacs --batch -l ~/.config/emacs/init.el
...
#+begin_src emacs-lisp
  ...
  (defvar scotty/gui-init-file
    (expand-file-name "gui.el" user-emacs-directory)
    "GUI customization init file.")

;;  (setq gui-init-file (expand-file-name "gui.el" user-emacs-directory))

  ;; try to load gui.el
  (when (and window-system (file-exists-p scotty/gui-init-file))
    (load scotty/gui-init-file))
...
#+begin_src emacs-lisp
  ...
  (setq lock-file-name-transforms `((".*" ,scotty/autosave-dir t)))
  ...
  (setq recentf-save-file
        (expand-file-name "recentf.eld" scotty/local-state-dir))
  ...
  (make-directory (expand-file-name "url" scotty/local-state-dir) t)
  (setq url-cookie-file (expand-file-name "url/cookie" scotty/local-state-dir))
...
[[https://codeberg.org/MonadicSheep/emacs-reader][emacs-reader]] supports viewing PDFs, ePUBs, .docx, etc. via [[https://mupdf.com/][mupdf]].
...
#+begin_src emacs-lisp
  (use-package projectile
    ...
    (projectile-frecency-file
     (expand-file-name "projectile-frecency.eld" scotty/local-state-dir))
...
=copilot-chat= enables chatting with Copilot
...
but is incompatible with =copilot=, which provides Copilot completion
...
#+begin_src emacs-lisp
  ...
  (use-package nodejs-repl
    ...
    :hook js2-mode-hook
    :custom
    (nodejs-repl-command (expand-file-name "~/.nodenv/shims/node")))
...
Also, add =gofmt-before-save= to the =before-save-hook= to cleanup Go files automatically
...
#+begin_src emacs-lisp
  (use-package ollama-buddy
    ...
    :defer t
    :custom
    (ollama-buddy-sessions-directory (expand-file-name "ollama-buddy-sessions" scotty/local-state-dir))
    (ollama-buddy-user-prompts-directory (expand-file-name "ollama-buddy-user-prompts" scotty/local-state-dir))
    :bind
...
#+begin_src emacs-lisp
  ...
  (use-package terraform-mode
    ...
    :init
    (setq flycheck-terraform-executable "tofu")
    :mode ("\\.tf\\'" "\\.tfvars\\'"))
...
#+begin_src emacs-lisp
  (use-package benchmark-init
    :ensure t
    :config
    ;; Disable collection after init is finished so it doesn't slow down normal operation
